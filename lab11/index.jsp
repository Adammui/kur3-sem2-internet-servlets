...
<script>
    ...
    document.getElementById("btn-submit-t2").addEventListener("click", function (event) {
        ...
        xhr.onreadystatechange = () => {
            if (xhr.readyState === 4) {
                ...
                let result = arr.map(number => number.innerHTML).join(" ");

                document.querySelector("#task2randomInts").value = result.toString();
                // альтернатвный способ
                // document.querySelector("#task2randomInts").value = xhr.responseXML.getElementsByTagName("num").item(1).textContent;
            }
        };
    ...
    document.getElementById("btn-submit-t3").addEventListener("click", function (event) {
        ...
        xhr.open('GET', '/lab11/SssJson');
        xhr.setRequestHeader("XRand-N", document.querySelector("#task2z").value);

        xhr.send();
        xhr.onreadystatechange = () => {
            if (xhr.readyState === 4) {
                document.querySelector("#task2randomInts").value = xhr.responseText.toString()
        ...
        if (isAsyncRequest) {
            xhr2.onreadystatechange = () => {
                if (xhr2.readyState === 4) {
                    console.log(stringifyXML(xhr2));
                    document.querySelector("#task4b").value = stringifyXML(xhr2).toString();
                    // альтернатвный способ
                    // document.querySelector("#task2randomInts").value = xhr.responseXML.getElementsByTagName("num").item(1).textContent;
                }
            };
        ...
        if (isAsyncRequest) {
            xhr3.onreadystatechange = () => {
                if (xhr3.readyState === 4) {
                    document.querySelector("#task4c").value = xhr3.responseText.toString()
                    //document.querySelector("#task2randomInts").value = stringifyJSON(xhr).toString();
                }
            };
    ...
    document.getElementById("btn-submit-t5").addEventListener("click", function (event) {
        $.ajax({
            ...
            success: function (result) {
                document.querySelector("#task2randomInts").value = result

            },
            error: function (error) {
                document.querySelector("#task2randomInts").value = JSON.parse(error.responseText)['res']
            }
